...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TFile *finput(0);
   TFile *pinput(0);
   //TString fname = "./tmva_class_example.root"; // ** Comente la linea original 
   TString fname = "../my-datasets/Fe.root"; // ** Agregue nuestro dataset
   TString pname = "../my-datasets/Po.root";
   if (!gSystem->AccessPathName( fname )) {
      finput = TFile::Open( fname ); // check if file in local directory exists
   }
   if (!gSystem->AccessPathName( pname )) {
      pinput = TFile::Open( pname ); // check if file in local directory exists
   }
   std::cout << fname << std::endl;
   std::cout << pname << std::endl;
   //else { // ** Comente la opcion de si no encuentra el dataset original lo descarga de una fuente
   ...
   std::cout << "--- TMVAClassification       : Using input file: " << finput->GetName() << std::endl;
   std::cout << "--- TMVAClassification       : Using input file: " << pinput->GetName() << std::endl;

   // Register the training and test trees
   // ** Ver la posibilidad de generar un root con datos de entrenamiento y prueba en trees separados
   TTree *signalTree     = (TTree*)finput->Get("Shower"); // ** Agregue los arboles de nuestros root donde estan lge y xmax
   TTree *background     = (TTree*)pinput->Get("Shower"); // ** Verificar la posibilidad de tener arboles con distinto nombre

   // Create a ROOT output file where TMVA will store ntuples, histograms, etc.
   TString outfileName( "TMVA-Fe.root" ); // ** Cambie el nombre del root de salida de resultados
   ...
   //dataloader->AddVariable( "myvar1 := var1+var2", 'F' ); // ** Estructura si se desea generar una variable combinada 
   //dataloader->AddVariable( "myvar2 := var1-var2", "Expression 2", "", 'F' ); // ** Estructura si se desea generar una variable combinada y con una expresion regular
   //dataloader->AddVariable( "var3",                "Variable 3", "units", 'F' ); // ** Estructura uso directo de una variable 
   //dataloader->AddVariable( "var4",                "Variable 4", "units", 'F' );
   dataloader->AddVariable( "lgE",                "variable lgE", "units", 'F' );
   dataloader->AddVariable( "Xmax",                "Variable Xmax", "units", 'F' );
   ...
   dataloader->AddSpectator( "spec1 := lgE",  "Spectator 1", "units", 'F' );
   dataloader->AddSpectator( "spec2 := Xmax",  "Spectator 2", "units", 'F' );
